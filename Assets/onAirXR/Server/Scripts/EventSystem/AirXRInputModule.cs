...
public class AirXRInputModule : PointerInputModule {
    ...
    private Dictionary<int, Dictionary<int, AirXRPointerEventData>> _pointerData = new Dictionary<int, Dictionary<int, AirXRPointerEventData>>();
    ...
    private void processAirXRInputEvents() {
        foreach (var pointer in AirXRPointer.pointers) {
            if (pointer.interactable) {
                var eventData = getAirXRPointerEventData(pointer);
                processMouseEvent(eventData);

                var leftButtonData = eventData.GetButtonState(PointerEventData.InputButton.Left).eventData.buttonData as AirXRPointerEventData;
