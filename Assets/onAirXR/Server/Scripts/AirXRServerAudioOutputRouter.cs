...
public class AirXRServerAudioOutputRouter : MonoBehaviour {
    ...
    public AirXRCameraRig targetCameraRig;
    ...
    void OnEnable() {
        if (input == Input.AudioPlugin && targetAudioMixer == null) {
            Debug.LogWarning("[WARNING] Please specify an audio mixer which includes an AirXRServerAudioOutputPlugin when input is AudioPlugin.");
        }

        if (output == Output.One && targetCameraRig == null) {
            Debug.LogWarning("[WARNING] Please specify an AirXRCameraRig when output is One.");
        }
        ...
        if (targetAudioMixer != null && targetAudioMixer.GetFloat(exposedRendererIDParameterName, out rendererID) == false) {
            Debug.LogWarning("[WARNING] Please expose the RendererID parameter of AirXR Server Audio Output plugin.");
        }
    }
    ...
    internal void SetOutputToOne(AirXRCameraRig targetCameraRig) {
        output = Output.One;
