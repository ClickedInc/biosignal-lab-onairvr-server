Shader "Unlit/FoveationVisualization"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float2 center = float2(0.5 + _GazeX / 2, 0.5 + _GazeY / 2);
                float distance = length(center - i.uv);

                return inrange(0, _InnerRadii / 2, distance) * fixed4(0, 0, 0, 0) + inrange(_InnerRadii / 2, _MidRadii / 2, distance) * _MidColor +
                       step(_MidRadii / 2, distance) * _OuterColor;
            }
            ...
            float tintOpacity(v2f i) 
            {
                ...
                return inrange(bound.x, bound.z, i.uv.x) * inrange(bound.w, bound.y, i.uv.y);
            }
